...
#we'll do a useless shiny page with several tests functionalities on it

#1 : create a verbatim formatted text

  shinyApp(
    ui = fluidPage(
      textInput("txt", "Enter your texte here:"),
      verbatimTextOutput("default"),
      textOutput("fancy")
    ),
    server = function(input, output) {
      output$default <- renderText({ input$txt })
      output$fancy <- renderText({ input$txt })
    }
  )
